  ...
  if (w > bh_n) {
    if (m->sel) {
      ...
      drw_text(drw, x, 0, w, bh,
               mid + (m->sel->icon ? m->sel->icw + ICONSPACING : 0),
               m->sel->name, 0);
      if (m->sel->icon)
...
void setup(void) {
  ...
  bh = user_bh ? user_bh : drw->fonts->h + user_bh + vertpadbar + borderpx * 2;
  th = vertpadtab;
  // bh_n = vertpadtab;
  updategeom();
  /* init atoms */
