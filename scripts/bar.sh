...
mem() {
  ...
  printf "^c$flamingo^ Mem:"
  printf "^c$text^ $mem_used_gb GB"
}

wlan() {
	case "$(cat /sys/class/net/wl*/operstate 2>/dev/null)" in
	up) printf "^c$flamingo^ 󰤨  ^d^%s" " ^c$text^Connected" ;;
	down) printf "^c$flamingo^ 󰤭  ^d^%s" " ^c$text^Disconnected" ;;
	esac
}

clock() {
	# printf "^c$text^ 󱑆  "
	printf "^c$flamingo^ $(date '+%Y-%m-%d %H:%M') "
}
